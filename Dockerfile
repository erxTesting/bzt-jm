# Taurus / JMeter only layered container	bzt-erx:v0.3 
...
RUN apt-get update \
    ...
    && bzt -o execution.0.concurrency=1 -o execution.0.iterations=1 \
	    -o settings.artifacts-dir="/var/tmp/jmeter/%Y-%m-%d_%H-%M-%S.%f" http://blazedemo.com/ \
